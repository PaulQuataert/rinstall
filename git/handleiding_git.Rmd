...
- Werkwijze voor Linux (wegens bug in RStudio):
    - Maak in RStudio een eenvoudig nieuw project aan
    - Volg de richtlijnen voor het synchroniseren van een bestaand project
...
- `git push origin --all` en geef je JIRA wachtwoord wanneer daarom gevraagd wordt.
...
- Nu opent Stash jouw kopie van de _repository_.
    - Klik op `Clone` en kopieer de URL.
- Open RStudio
- Onder Windows
    - Maak een nieuw project en kies voor versiebeheer en Git 
    - Plak de URL die je in Stash kopieerde in het vak `Repository URL`
    - RStudio zal automatisch de naam van de _repository_ voorstellen als projectnaam. Je kan dit, indien gewenst, aanpassen. De voorgestelde naam behouden heeft als voordeel dat je minder verwarring krijgt.
    - Geef de gewenste map waarin het project moet komen en klik op `Create project`
    - Jouw project wordt nu gesynchoniseerd met de _repository_ op Stash
- Onder Linux
    - Maak een project in een nieuwe directory. Kies een leeg project en vink Git aan. Geef het voor het gemak dezelfde naam als jouw _fork_ van de _repository_.
    - Ga naar de `shell` en voer het commando `git remote add origin STASHURL` uit. Hierbij moet je `STASHURL` vervangen door de URL die je op de Stash website gekopieerd hebt.
    - `git fetch origin`


De laatste stap is het maken van de gewenste lokale _branches_. Hieronder de code voor de **master** _branch_ doe dat voor alle _branches_ waar je lokaal mee verder wilt werken.
- `git checkout --track origin/master`


Nu beschik je over een lokale kopie. De versie op Stash zal automatisch bijgewerkt worden als de bron _repository_ aangepast wordt. Die wijzingen worden echter niet automatisch naar jouw lokale versie doorgestuurd. Die wijzigingen moet je manueel binnenhalen met `git fetch origin`
