...
## Handige dingen om eenmalig in te stellen
...
`git config --global core.editor c:/R/RStudio/bin/rstudio.exe`
  ~ gebruik RStudio als standaard editor
...
`git config --global credential.helper cache`
  ...
  ~ als je liever hebt dat je wachtwoord permanent opgeslagen wordt, kan je de tool installeren van https://gitcredentialstore.codeplex.com/
...
Een lokaal project staat enkel op jouw computer. Je hebt dus geen servers of andere computers nodig om versiebeheer te gebruiken! Een eigen lokaal project is prima om te leren werken met git. Je kan alles doen, behalve de stappen die verband houden met het synchroniseren tussen computers / servers.
...
- Wanneer _ignore_ gebruiken
    - Vuistregel: alle bestanden die je makkelijk kan aanmaken met de gegevens in de _repository_
...
- Eens alle gewenste bestanden _gestaged_ zijn, kan je _committen_ door op de knop _commit_ te klikken.
    ...
    - Onderaan krijg je de _diff_ te zien van het bestand. Groene regels zijn toegevoegd, rode regels zijn gewist, witte regels zijn ongewijzigd.
        ...
        - De hoeveelheid context kan je aanpassen via de drop-down `Context` (5, 10, 25, 50 of alle lijnen)
...
### Bestanden wissen en terug zetten naar de laatste _commit_
...
### Selectief wijzigingen _comitten_

- Heb je toevallig al meer wijzingen aangebracht dan hetgeen in een _commit_ wou steken? Geen probleem. RStudio maakt het relatief eenvoudig om een gedeelte van een bestand te _stagen_
- Wijzig `script2.R` naar onderstaande code

```{r script2_bis, eval = FALSE}
ctl <- c(4.17,5.58,5.18,6.11,4.50,4.61,5.17,4.53,5.33,5.14)
trt <- c(4.81,4.17,4.41,3.59,5.87,3.83,6.03,4.89,4.32,4.69)
...
- Klik op de _commit_ knop in het _git_ venster, zonder eerst de wijzigingen te _stagen_.
- In het venster met de wijzigingen zal je rechtboven elke _chunk_ de knoppen '_Stage chunk_' en '_Discard chunk_' vinden.
    - '_Stage chunk_' zal enkel de wijzigingen in de betrokken _chunk_ stagen. De rest van het bestand wordt niet _gestaged_.
        - Je kan uiteraard meerdere _chunk_ uit een bestand _stagen_. Alle _chunks_ _stagen_ is hetzelfde als het volledige bestand _stagen_.
    - 'Discard chunk' verwijdert de wijzigingen in deze _chunk_. **OPGELET** dit is permanent en onherroepelijk. Er wordt geen bevestiging gevraagd!
    - Wat is een _chunk_: alle wijzigingen die een blok vormen waarbij het aantal ongewijzigde lijnen tussen twee wijzigingen bepalen wat de grens is. Zolang het aantal ongewijzigde lijnen kleiner is dan het aantal vermeld bij 'Context' (standaard 5 lijnen), behoren de wijzigingen tot eenzelfde chunk. Je kan dus de grootte van een chunk beïnvloeden door de 'Context' te wijzigen.
- Nog fijnere controle
    - Als je op een gewijzigde lijn klikt, verschijnen rechts twee nieuwe knoppen '_Stage line_' en '_Discard line_'. Hun werking is vergelijkbaar aan '_Stage chunk_' en '_Discard chunk_' maar dan enkel toegepast op de geselecteerde lijn.
    - Door de 'Shift' toets in te drukken en een tweede lijn te selecteren, selecteer je alle wijzigingen tussen deze twee lijnen. Nu krijg je de knoppen _Stage selection_' en '_Discard selection_'.
    - Door de 'Ctrl' toets in te drukken en op een lijn de klikken, voeg je die lijn toe aan de selectie of haal je ze weer uit de selectie.
- Van zodra je een _chunk_ of een (selectie van) lijn(en) _staget_ en er zijn nog _unstaged_ wijzigingen, zal verdwijnen de _gestagede_ wijzigingen uit de viewer. Om te zien wat je juist _gestaged_ hebt, kan je op de radiobutton '_Staged_' klikken (links bovenaan de viewer). 
    - Hier krijg je de mogelijkheden om de wijzigingen terug te _unstagen_.
- _Stage_ een deel van de wijzigingen aan `script2.R` en _commit_.
- _Stage_ en _commit_ vervolgens de rest van de wijzigingen aan `script2.R`.
...
- Nu gaan we de wijzigingen in **develop** als een blok toepassen op **master**
    ...
    - Je krijgt een editor venster waarin je een aangepaste _commit message_ schrijft. Hier beschijf je beknopt de belangrijkste wijzigingen sinds de vorige _merge_ tussen **master** en **develop**
...
- Vervolgens **featureC**
    ...
    - We negeren even de wijzigen van de huidige onafgewerkte _merge_ met `git reset --hard`. Dit command zet **_alle_** wijzigingen terug naar de toestand van de laatste _commit_. `featureC.R` zat nog niet in **develop** en wordt daarom _untracked_.
...
Dit is de werkwijze die je best volgt als je een nieuw project wilt starten en er dadelijk ook een kopie van op Stash wenst. Dat is handig als je de optie wilt hebben om de inhoud vlot te delen met anderen. Het delen is overigens geen verplichting!
...
- Werkwijze voor Windows: 
    ...
    - RStudio zal je wachtwoord voor Stash vragen (= JIRA account).
    - Jouw project wordt nu gesynchroniseerd met de (lege) _repository_ op Stash
...
- Klik in het linkermenu op `Fork`
    ...
    - `Enable fork syncing geactiveerd` is standaard geactiveerd. Hiermee blijft jouw kopie op Stash automatisch gesynchroniseerd met de bron.
...
- Stel dat we een aanpassing willen doen aan **les2**
    - Ga na of **les2** bestaat. Zoniet `git checkout --track origin/les2`
    - We doen eerst een _checkout_ van **les2**
...
- Nu kunnen we onze nieuwe _branch_ _pushen_ naar je eigen _fork_
